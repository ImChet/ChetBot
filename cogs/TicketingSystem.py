        ...
        # No cooldown, continue
            ...
            if ticket is not None:
                ...
                if not category_exists:
                    category = await interaction.guild.create_category(f'TICKETS', reason='Ticketing category made by ChetBot')
        ...
        if os.path.exists(transcript_file):
            ...
            with open(transcript_file, "a") as transcript:
                ...
                async for message in interaction.channel.history(limit=None, oldest_first=True):
                    counter += 1
                    # Ignore the first message in a ticket, just information sent by ChetBot that we don't need
                    if counter not in [1]:
...
class TicketingSystem(commands.Cog, name='Ticketing System', description='Ticketing System'):
    ...
    async def remove(self, interaction: discord.Interaction, member: discord.Member):
        if 'ticket-for-' in interaction.channel.name:
            ...
            await interaction.response.send_message(f'This channel is not a ticket. I am only able to remove members from a ticket.')


async def setup(ChetBot):
    await ChetBot.add_cog(TicketingSystem(ChetBot))
